...
abstract class EnergyCalculator
{
    ...
    public function __construct(House $house, Parameters $parameters, SolarPanelCalculatorService $solarPanelCalculatorService)
    {
        $this->house        = $house;
    ...
    public function subtractRenewable(Renewable $renewable)
    {
        ...
        switch ($renewable->getSlug()) {
            case Renewable::RENEWABLE_SOLAR_WATER_HEATER:
                $amount *= $this->house->getOccupants();
                $fromActual = false;
                $forceElectric = false;
                break;
            case Renewable::RENEWABLE_SOLAR_PANELS:
                $amount = $this->solarPanelCalculatorService->getSolarPanelEnergyAmount($this->house, $amount);
                break;
     ...
     * @return array|\One\CheckJeHuis\Entity\Config[][][]
     */
